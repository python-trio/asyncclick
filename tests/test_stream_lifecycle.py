...
from asyncclick.testing import _NamedTextIOWrapper
from asyncclick.testing import BytesIOCopy
from asyncclick.testing import CliRunner
from asyncclick.testing import StreamMixer
...
@pytest.mark.anyio
async def test_wrapper_close_does_not_close_underlying_buffer():
...
@pytest.mark.anyio
async def test_wrapper_del_does_not_close_underlying_buffer():
...
@pytest.mark.anyio
async def test_multiple_wrappers_same_buffer():
...
@pytest.mark.anyio
async def test_wrapper_preserves_name_and_mode():
    buf = io.BytesIO()
...
@pytest.mark.anyio
async def test_mixer_buffers_survive_wrapper_gc():
...
@pytest.mark.anyio
async def test_getvalue_after_isolation_exit():
...
@pytest.mark.anyio
async def test_no_streammixer_del():
...
@pytest.mark.anyio
async def test_bytesiocopy_writes_to_both():
...
@pytest.mark.anyio
async def test_bytesiocopy_flush_propagates():
...
@pytest.mark.anyio
async def test_invoke_with_logger_warning():
...
@pytest.mark.anyio
async def test_invoke_with_logger_and_prompt():
    ...
    runner = CliRunner()
    result = await runner.invoke(hello, input="Peter")
    assert result.exit_code == 0
    assert "Your name:" in result.output


@pytest.mark.anyio
async def test_sequential_invokes_with_logging():
...
@pytest.mark.anyio
async def test_logging_with_cli_log_level():
...
@pytest.mark.anyio
async def test_invoke_with_thread_pool():
...
@pytest.mark.anyio
async def test_invoke_with_threads_writing_to_streams():
...
@pytest.mark.anyio
async def test_invoke_with_thread_pool_and_exit():
...
@pytest.mark.anyio
async def test_sequential_threaded_invokes():
    ...
    runner = CliRunner()
    for i in range(10):
        result = await runner.invoke(cli, [str(i)])
        assert result.exit_code == 0
...
@pytest.mark.anyio
async def test_output_isolation_across_invokes():
    ...
    for word in ("alpha", "beta", "gamma"):
        result = await runner.invoke(cli, [word])
        assert result.exit_code == 0
        assert result.output.strip() == word


@pytest.mark.anyio
async def test_stderr_isolation_across_invokes():
...
@pytest.mark.anyio
async def test_mixed_output_order_preserved():
...
@pytest.mark.anyio
async def test_exception_does_not_corrupt_next_invoke():
    ...
    r1 = await runner.invoke(cli)
    assert r1.exit_code == 0
    assert "call-1" in r1.output

    r2 = await runner.invoke(cli)
    assert r2.exit_code == 1
    assert isinstance(r2.exception, RuntimeError)

    r3 = await runner.invoke(cli)
    assert r3.exit_code == 0
    assert "call-3" in r3.output


@pytest.mark.anyio
async def test_sys_streams_restored_after_invoke():
...
@pytest.mark.anyio
async def test_sys_streams_restored_after_exception():
...
@pytest.mark.anyio
async def test_stress_thread_pool_with_exit(_):
...
@pytest.mark.parametrize("_", range(10_000))
@pytest.mark.anyio
async def test_stress_logging_sequential_invocations(_):
...
@pytest.mark.parametrize("_", range(10_000))
@pytest.mark.anyio
async def test_stress_gc_between_invocations(_):
